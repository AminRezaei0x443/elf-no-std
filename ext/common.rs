...
macro_rules! define_benchmark {
    (
        ...
        #[global_allocator]
        static mut GLOBAL_ALLOC: alloca::SimpleAlloc<{ $heap_size }> = alloca::SimpleAlloc::new();
...
macro_rules! define_benchmark {
    (
        ...
        unsafe impl alloca::alloc::GlobalAlloc for SolanaAlloc {
            #[inline]
            unsafe fn alloc(&self, layout: alloca::alloc::Layout) -> *mut u8 {
                let heap = 0x300000000 as *mut SolanaHeap<{$heap_size}>;
                alloca::alloc::GlobalAlloc::alloc(&(*heap).allocator, layout)
            }

            #[inline]
            unsafe fn dealloc(&self, pointer: *mut u8, layout: alloca::alloc::Layout) {
                let heap = 0x300000000 as *mut SolanaHeap<{$heap_size}>;
                alloca::alloc::GlobalAlloc::dealloc(&(*heap).allocator, pointer, layout)
            }
        }
